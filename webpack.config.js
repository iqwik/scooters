...
if (!isDev)
{
    ...
    const prodAssets = path.join(prodPath, 'assets');

    fs.removeSync(prodPath);

    fs.mkdirSync(prodPath, err => console.error(err));
    fs.mkdirSync(prodAssets, err => console.error(err));
    fs.mkdirSync(path.join(prodAssets, 'img'), err => console.error(err));
    fs.mkdirSync(destination, err => console.error(err));
    fs.copy(source, destination, err => console.error(err)||console.log('\nCopy completed!'));
}
...
module.exports = {
    ...
    plugins: [
        ...
        new HtmlWebpackPlugin({
            template: path.join(devPath, '/html/index.html'),
        })
    ],
    module:
    {
